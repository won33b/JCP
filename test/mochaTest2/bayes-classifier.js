...
/*********************************************************************************** */
 BayesClassifier.prototype.inputData = function(add1, add2, label)
 {
    addPath = path.join(__dirname,add1,add2);
    var files = fs.readdirSync(addPath);
    for(i=0;i<files.length;i++)
    {
      var array = fs.readFileSync(addPath+'/'+files[i]).toString().split('\n')
      this.addDocuments(array,label)
    }
  };
 /*****************************************************************************************************2021-11-26추가**** */
 /*
...
for(i=0;i<files.length;i++)
{
  ...
  classifier.addDocuments(string, "NotAd");
